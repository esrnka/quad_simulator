...
%
%    eaVec --- 4x1 vector of voltages applied to motors, in rad/sec.  eaVec(i)
...
if(norm(vI) > epsilon_vI)
  vIunit = vI/norm(vI);
  da = P.quadParams.Cd*P.quadParams.Adisk*zI'*vIunit;
else
  da = 0;
end
...
rIdot = vI;
vIdot = ([0;0;-mq*gE] + RBI'*sum(FMat,2) + distVec - da*vI)/mq;
RBIdot = -omegaBx*RBI;
NB = sum(NMat,2);
for ii=1:4
