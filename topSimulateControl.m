...
% simulateQuadrotorEstimationAndControl. This version controls the UKF
% simulator
...
% Oversampling factor
S.oversampFact=2;
% Camera feature locations in the I frame. Add more for better camera
% performance. Minimum of 1 feature required for estimator convergence.
...
S2.gifFileName = 'UKF.gif';
% These bounds are hardcoded to match the planned path and may need to
% change depending on path
S2.bounds=5*[-.6 .6 -.6 .6 -0.15 1.1];
visualizeQuad(S2);

% Plot of norm of lever arm covariance to ensure estimator is converging on
% a good estimate
figure(3); plot(tVec(1:end-1),Q.state.PlB); 
xlabel('t (s)'); ylabel('Norm of P_{lB}');
